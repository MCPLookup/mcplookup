...
export class StatusCommand extends BaseCommand {
    ...
    displayStatus(status) {
        ...
        if (status.servers.bridge.length > 0) {
            console.log('\n🔗 Bridge Mode Servers:');
            const bridgeTable = status.servers.bridge.map((server) => ({
                Name: server.name,
        ...
        if (status.servers.direct.length > 0) {
            ...
            const directTable = status.servers.direct.map((server) => ({
                Name: server.name,
        ...
        const runningServers = status.servers.bridge.filter((s) => s.status === 'running').length;
        const errorServers = status.servers.bridge.filter((s) => s.status === 'error').length;
        console.log('\n📈 Summary:');
