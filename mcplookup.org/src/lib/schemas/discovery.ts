...
// Verification challenge schema for DNS verification
export const VerificationChallengeSchema = z.object({
  challenge_id: z.string().optional().describe("Challenge ID"),
  domain: SecureDomainSchema.describe("Domain to verify"),
  challenge_token: z.string().describe("DNS verification token"),
  verification_type: z.enum(['dns-txt', 'dns-cname']).default('dns-txt').describe("Verification method"),
  expires_at: z.string().datetime().describe("Challenge expiration time"),
  txt_record_name: z.string().optional().describe("DNS TXT record name"),
  txt_record_value: z.string().optional().describe("DNS TXT record value"),
  instructions: z.string().optional().describe("Verification instructions"),
  status: z.enum(['pending', 'verified', 'expired']).optional().describe("Verification status")
});
