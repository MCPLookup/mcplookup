...
export class DiscoveryService {
  ...
  /**
   * Discover a server by domain name
   * Simple wrapper around discoverServers for backward compatibility
   */
  async discoverByDomain(domain: string): Promise<MCPServer | null> {
    const response = await this.discoverServers({ domain, limit: 1 });
    return response.servers.length > 0 ? response.servers[0] : null;
  }
