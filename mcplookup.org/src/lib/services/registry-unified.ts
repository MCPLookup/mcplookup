...
export class RegistryService implements IRegistryService {
  ...
  async getServersByDomain(domain: string): Promise<MCPServerRecord[]> {
    ...
    return result.data.items; // Get items from paginated result
  }
  ...
  async updateServer(domain: string, updates: Partial<MCPServerRecord>): Promise<void> {
    ...
    if (!isSuccessResult(existingResult)) {
      throw new Error(`Failed to get existing server: ${existingResult.error}`);
    }
