...
export class DockerManager {
     ...
     */
    async dockerizeNpmServer(server, env = {}) {
     ...
     */
    createNpmDockerCommand(packageName, options = {}) {
        const { containerName = `mcp-${packageName.replace(/[@\/]/g, '-')}`, mode = 'bridge', env = {}, includePortMapping = false } = options;
        // Base Docker command
        const baseCommand = [
            'docker', 'run', '--rm', '-i',
            '--name', containerName,
     ...
     */
    createDirectModeDockerArgs(packageName, env = {}) {
     ...
     */
    addResourceLimits(command, options = {}) {
     ...
     */
    addEnvironmentVariables(command, env) {
        const envOptions = [];
     ...
     */
    createOptimizedCommand(server, env = {}) {
